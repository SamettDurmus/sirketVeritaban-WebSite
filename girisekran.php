...
//     $sql="select * from admin where admin_id='".$uname."' AND parola='".$password."' limit 1";
...
if(isset($_POST['username'])){
    ...
    $sql="select * from admin where admin_id='".$uname."' limit 1";
    $result = pg_query($con,$sql);

    if(pg_num_rows($result)==1){
        $ilgilikayit = pg_fetch_assoc($result);
        $hashlisifre = $ilgilikayit["parola"];

        if(password_verify($password,$hashlisifre)){
            session_start();
            $_SESSION['uname'] = $ilgilikayit["admin_id"];
